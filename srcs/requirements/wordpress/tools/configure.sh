...
# Add the following to create a new normal user:
wp user create "$WP_USER_NORMAL" normal@mail.com --role=author --user_pass="$WP_PASSWORD_NORMAL" --path=/tmp/wordpress --allow-root
